...
train_data['5*10'] = train_data['Parameter5'] * train_data['Parameter10']
test_data['5*10'] = test_data['Parameter5'] * test_data['Parameter10']
...
def my_loss(y_true, y_pred):
	alpha = [1., 1., 1., 1.]
	gamma = 2.
	y_true = OneHotEncoder(n_values=4, sparse=False).fit_transform(y_true.reshape(-1, 1))
	loss = np.zeros((1, y_true.shape[1]))
	for label in range(y_true.shape[1]):
		temp = -alpha[label] * ((1 - y_true[:, label]) ** gamma) * np.log(y_pred[:, label])
		loss[0, label] = np.sum(temp)
	return np.sum(loss)

class Cat_Loss(object):
	def my_loss_cat(self, y_pred, y_true, weights):
		alpha = [1., 1., 1., 1.]
		gamma = 2.
		y_true = OneHotEncoder(n_values=4, sparse=False).fit_transform(y_true.reshape(-1, 1))
		loss = np.zeros((1, y_true.shape[1]))
